...
for route in routeList: #shape
    ...
    for subRoute in data:
        innerCoor=[]
        subRouteName=subRoute["SubRouteName"].get("Zh_tw")
        if not subRouteName:
            subRouteName=subRoute["RouteName"]["Zh_tw"]
        newdic={"SubRoute":subRouteName,"Direction":subRoute["Direction"],"Geometry":innerCoor}
